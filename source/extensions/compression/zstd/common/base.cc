...
Base::Base(uint32_t chunk_size)
    : chunk_size_(chunk_size), chunk_ptr_{std::make_unique<uint8_t[]>(chunk_size)},  input_ptr_{std::make_unique<uint8_t[]>(chunk_size)},
    input_len_(0), output_{chunk_ptr_.get(), chunk_size, 0}, input_{nullptr, 0, 0} {}

void Base::setInput() {
  input_.src = input_ptr_.get();
  input_.pos = 0;
  input_.size = input_len_;
  input_len_ = 0;
}
